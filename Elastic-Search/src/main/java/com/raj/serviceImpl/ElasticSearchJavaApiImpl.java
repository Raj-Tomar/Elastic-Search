...
public class ElasticSearchJavaApiImpl implements ElasticSearchJavaApi {
	...
	public ResponseEntity<String> TestES() {
		logger.info("TestES");
		ResponseEntity<String> response = ElasticSearchServiceImpl.elasticSearchCall(ELASTIC_SEARCH_URL, HttpMethod.GET, null);
		return response;
	}
	...
	public ResponseEntity<String> filter(String search) throws UnknownHostException {
		...
		SearchResponse res = client.prepareSearch("movies").setQuery(qb).execute().actionGet();
